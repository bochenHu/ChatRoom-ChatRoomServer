...
void MainWin::listWidgetMenuClicked()
{
    ...
    if( act != NULL )
    {
        ...
        if( sl.length() > 0 )
        {
            ...
            QString tip = "check to execute the " + act->text() + " action for [ " + user + " ] ?" ;

            if( QMessageBox::question(this, "Hint", tip, QMessageBox::Yes, QMessageBox::No) == QMessageBox::Yes )
            {
        ...
        {
            QMessageBox::information(this, "Hint", "Please select chatting members");
        }
    }
}
...
void MainWin::logInOutBtnClicked()
{
    if( !m_client.isValid() )
    {
        ...
        if( loginDlg.exec() == QDialog::Accepted )
        {
            ...
            {
                QMessageBox::critical(this, "Failed", "cannot connect to the server");
            }
        }
    }
    else
...
void MainWin::LIOK_Handler(QTcpSocket&, TextMessage& message)
{
    ...
    else if( status == "kick" )
    {
        ...
        QMessageBox::information(this, "Hint", "Account member [ " + id + " ] was banned to log in");
    }
    else
    {
        setCtrlEnabled(true);
...
void MainWin::CTRL_Handler(QTcpSocket&, TextMessage& message)
{
    ...
    {
        QMessageBox::information(this, "Hint", "You have been muted by the administrator!");
    ...
    {
        QMessageBox::information(this, "Hint", "The administrator has restored your chat rights!");
    ...
    {
        QMessageBox::information(this, "Hint", "Account member [ " + inputGrpBx.title() + " ] not allowed to log in to the chat room!");
